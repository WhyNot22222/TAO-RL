...
def compute_policy_loss(old_log_prob,
    ...
    standard_clipped_ratio = torch.clamp(ratio, 1.0 - cliprange_low, 1.0 + cliprange_high)
    if use_aepo_anti_clip and aepo_anti_clip_mask is not None:
        # AEPO anti-clipping only applies to selected tokens.
        anti_clip_mask = aepo_anti_clip_mask.bool() & eos_mask.bool()
        upper_bound_ratio = (1.0 + cliprange_high) / ratio.detach() * ratio
        clipped_ratio = torch.where(
            anti_clip_mask & (ratio > 1.0 + cliprange_high),
            upper_bound_ratio,
            standard_clipped_ratio,
        )
    else:
        clipped_ratio = standard_clipped_ratio
