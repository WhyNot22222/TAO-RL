...
EXEC_ERROR_FILTER_ENABLE=False
EXEC_ERROR_FILTER_STRENGTH=1.0
EXEC_ERROR_FILTER_SCOPE=failed
EXEC_ERROR_FILTER_DEBUG=False
EXEC_ERROR_FILTER_DEBUG_DIR=null
EXEC_ERROR_FILTER_DEBUG_MAX_ROWS=0
OVERSAMPLE=3
VAL_ONLY=False
LOG_VAL_GENERATIONS=64
OUTPUT_ACC_TO_FILE=False
ENTROPY_COEFF=0.001
ENTROPY_PERCENTILE=0.7
USE_AEPO_SHAPING=False
USE_AEPO_ANTI_CLIP=False
AEPO_SHAPING_COEFF=0.01
DATA_SHUFFLE=True
...
generate_suffix() {
  ...
  while [[ "$#" -gt 0 ]]; do
    case $1 in
      ...
      --exec_error_filter_enable) suffix+="_execerrfilter$2"; shift 2 ;;
      --exec_error_filter_strength) suffix+="_execerrstrength$2"; shift 2 ;;
      --exec_error_filter_scope) suffix+="_execerrscope$2"; shift 2 ;;
      --exec_error_filter_debug) suffix+="_execerrdebug$2"; shift 2 ;;
...
while [[ "$#" -gt 0 ]]; do
  ...
  case "$1" in
    ...
    --exec_error_filter_enable) EXEC_ERROR_FILTER_ENABLE="$2"; shift 2 ;;
    --exec_error_filter_strength) EXEC_ERROR_FILTER_STRENGTH="$2"; shift 2 ;;
    --exec_error_filter_scope) EXEC_ERROR_FILTER_SCOPE="$2"; shift 2 ;;
    --exec_error_filter_debug) EXEC_ERROR_FILTER_DEBUG="$2"; shift 2 ;;
    --exec_error_filter_debug_dir) EXEC_ERROR_FILTER_DEBUG_DIR="$2"; shift 2 ;;
    --exec_error_filter_debug_max_rows) EXEC_ERROR_FILTER_DEBUG_MAX_ROWS="$2"; shift 2 ;;
    ...
    --entropy_percentile) ENTROPY_PERCENTILE="$2"; shift 2 ;;
    --use_causal_entropy) shift 2 ;;
    --use_aepo_shaping) USE_AEPO_SHAPING="$2"; shift 2 ;;
    --use_aepo_anti_clip) USE_AEPO_ANTI_CLIP="$2"; shift 2 ;;
    --aepo_shaping_coeff) AEPO_SHAPING_COEFF="$2"; shift 2 ;;
    --shuffle) DATA_SHUFFLE="$2"; shift 2 ;;
    --data.shuffle) DATA_SHUFFLE="$2"; shift 2 ;;
    *)
...
echo "Exec Error Filter Enable: $EXEC_ERROR_FILTER_ENABLE"
echo "Exec Error Filter Strength: $EXEC_ERROR_FILTER_STRENGTH"
echo "Exec Error Filter Scope: $EXEC_ERROR_FILTER_SCOPE"
echo "Exec Error Filter Debug: $EXEC_ERROR_FILTER_DEBUG"
echo "Exec Error Filter Debug Dir: $EXEC_ERROR_FILTER_DEBUG_DIR"
echo "Exec Error Filter Debug Max Rows: $EXEC_ERROR_FILTER_DEBUG_MAX_ROWS"
...
sleep 3
PYTHONUNBUFFERED=1 python -m recipe.tao_rl.main_tao_rl \
    ...
    trainer.default_local_dir=$CHECKPOINT_PATH/$CKPT_NAME \
    trainer.total_epochs=$TOTAL_EPOCHS \
    trainer.resume_mode=$RESUME_MODE \
    trainer.resume_from_path=$RESUME_FROM_PATH \
    trainer.balance_batch=$BALANCE_BATCH \
    trainer.exec_error_filter.enable=$EXEC_ERROR_FILTER_ENABLE \
    trainer.exec_error_filter.strength=$EXEC_ERROR_FILTER_STRENGTH \
    trainer.exec_error_filter.scope=$EXEC_ERROR_FILTER_SCOPE \
    trainer.exec_error_filter.debug=$EXEC_ERROR_FILTER_DEBUG \
    trainer.exec_error_filter.debug_dir=$EXEC_ERROR_FILTER_DEBUG_DIR \
    trainer.exec_error_filter.debug_max_rows=$EXEC_ERROR_FILTER_DEBUG_MAX_ROWS \
    ...
    actor_rollout_ref.actor.causal_entropy.enable=$USE_AEPO_SHAPING \
    actor_rollout_ref.actor.causal_entropy.use_aepo_shaping=$USE_AEPO_SHAPING \
    actor_rollout_ref.actor.causal_entropy.entropy_percentile=$ENTROPY_PERCENTILE \
    actor_rollout_ref.actor.causal_entropy.use_aepo_anti_clip=$USE_AEPO_ANTI_CLIP \
    actor_rollout_ref.actor.causal_entropy.aepo_shaping_coeff=$AEPO_SHAPING_COEFF \
