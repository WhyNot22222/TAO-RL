...
from recipe.tao_rl.agent_utils import AgentHelper, GenerationConfig
from recipe.tao_rl.utils.dataset.rl_dataset import RLCustomPromptDataset
...
def apply_kl_penalty(
    ...
    elif "response_mask" in data.batch.keys():
        response_mask = data.batch["response_mask"]
    elif mask_tool_output:
...
# begin: causal influence from advantage
def _masked_minmax_normalize(
    x: torch.Tensor, mask: torch.Tensor, eps: float
) -> torch.Tensor:
    ...
    for i in range(x.size(0)):
        ...
        if (hi - lo).abs() < eps:
            ...
            out[i, valid] = (xv - lo) / (hi - lo + eps)
    return out


def attach_causal_influence(batch: DataProto, ce_cfg) -> dict:
    response_mask = batch.batch["response_mask"].float()
    eps = float(ce_cfg.get("eps", 1.0e-6))
    power = float(ce_cfg.get("influence_power", 1.0))

    raw = batch.batch["advantages"] # no abs version
    raw = raw.pow(power) * response_mask

    if ce_cfg.get("normalize_influence", True):
        influence = _masked_minmax_normalize(raw, response_mask, eps)
    else:
        influence = raw

    influence = influence * response_mask
    batch.batch["causal_influence"] = influence

    return {
        "actor/causal_influence_mean": masked_mean(influence, response_mask).item(),
        "actor/causal_influence_max": influence.max().item(),
    }

def attach_traj_explore_mask(batch: DataProto, ce_cfg) -> dict:
    """Select top trajectories per prompt group and expand to token-level mask."""
    response_mask = batch.batch["response_mask"].float()
    advantages = batch.batch["advantages"]

    keep_ratio = float(ce_cfg.get("trajectory_keep_ratio", 0.875))
    min_keep = int(ce_cfg.get("trajectory_min_keep", 1))
    eps = float(ce_cfg.get("eps", 1.0e-6))

    traj_scores = masked_mean(advantages, response_mask, axis=-1)  # [bs]
    traj_select = torch.zeros_like(traj_scores, dtype=torch.float32)
    ...
    for _, idx_list in group_to_indices.items():
        group_scores = traj_scores[idx_list]
        group_size = len(idx_list)
        keep_k = max(min_keep, int(np.ceil(group_size * keep_ratio - eps)))
        keep_k = min(keep_k, group_size)
        top_local = torch.topk(group_scores, k=keep_k, largest=True).indices.tolist()
        for local_i in top_local:
            traj_select[idx_list[local_i]] = 1.0

    traj_explore_mask = traj_select.view(-1, 1).expand_as(response_mask) * response_mask
    batch.batch["traj_explore_mask"] = traj_explore_mask

    return {
        "actor/traj_explore_ratio": masked_mean(traj_explore_mask, response_mask).item(),
        "actor/traj_score_mean": traj_scores.mean().item(),
        "actor/traj_score_max": traj_scores.max().item(),
    }
...
class RayTAORLTrainer(RayPPOTrainer):
    """
    ...
    def _create_dataloader(self):
        ...
        if self.config.data.shuffle:
            ...
            print(f"[DataLoader] Training data shuffle: True (random)")
        else:
            sampler = SequentialSampler(data_source=self.train_dataset)
            print(f"[DataLoader] Training data shuffle: False (sequential)")
    ...
    def _validate(self):
        ...
        # Lists to collect additional metrics
        all_tool_call_counts = []
        all_code_lines = []
        all_response_lengths = []  # in characters
        all_response_length_tokens = []  # in tokens
        all_turn_counts = []
        ...
        for test_data in self.val_dataloader:
            ...
            # Calculate additional metrics for each output
            for i, output_text in enumerate(output_texts):
                # Count code blocks and tool calls
                code_block_pattern = r'```(?:py|python)?\n(.*?)\n```'
                code_blocks = re.findall(code_block_pattern, output_text, re.DOTALL)
                tool_calls = len(code_blocks)
                all_tool_call_counts.append(tool_calls)

                # Count code lines
                total_code_lines = sum(len(block.strip().split('\n')) for block in code_blocks)
                all_code_lines.append(total_code_lines)

                # Response length in characters and tokens
                response_length_chars = len(output_text)
                all_response_lengths.append(response_length_chars)

                # Count tokens (using attention_mask to get actual length, not padded length)
                response_length_tokens = attention_mask[i].sum().item()
                all_response_length_tokens.append(response_length_tokens)

                # Count turns (based on tool calls or other indicators)
                if self.config.agent.tool_use:
                    turns = tool_calls + 1  # Each tool call is a turn, plus final response
                else:
                    turns = 1
        ...
        for data_source, rewards in data_source_reward.items():
            ...
            # Save per-sample scores for combining multiple runs (e.g., two k=32 -> k=64)
            if self.config.trainer.get('val_only', False):
                save_per_sample_path = self.config.trainer.get('save_per_sample_path', None)
                if save_per_sample_path:
                    import os
                    base, ext = os.path.splitext(str(save_per_sample_path))
                    save_path = f"{base}_{data_source}{ext}"
                    os.makedirs(os.path.dirname(save_path), exist_ok=True)
                    np.save(save_path, reward_per_test_sample)
                    print(f"[{data_source}] Per-sample scores saved to {save_path}")
        ...
        if all_tool_call_counts:
            metric_dict["val/avg_tool_calls"] = np.mean(all_tool_call_counts)
            metric_dict["val/max_tool_calls"] = np.max(all_tool_call_counts)
            metric_dict["val/min_tool_calls"] = np.min(all_tool_call_counts)
            metric_dict["val/std_tool_calls"] = np.std(all_tool_call_counts)

        if all_code_lines:
            metric_dict["val/avg_code_lines"] = np.mean(all_code_lines)
            metric_dict["val/max_code_lines"] = np.max(all_code_lines)
            metric_dict["val/min_code_lines"] = np.min(all_code_lines)
            metric_dict["val/std_code_lines"] = np.std(all_code_lines)

        if all_response_lengths:
            metric_dict["val/avg_response_length_chars"] = np.mean(all_response_lengths)
            metric_dict["val/max_response_length_chars"] = np.max(all_response_lengths)
            metric_dict["val/min_response_length_chars"] = np.min(all_response_lengths)
            metric_dict["val/std_response_length_chars"] = np.std(all_response_lengths)

        if all_response_length_tokens:
            metric_dict["val/avg_response_length_tokens"] = np.mean(all_response_length_tokens)
            metric_dict["val/max_response_length_tokens"] = np.max(all_response_length_tokens)
            metric_dict["val/min_response_length_tokens"] = np.min(all_response_length_tokens)
            metric_dict["val/std_response_length_tokens"] = np.std(all_response_length_tokens)

        if all_turn_counts:
            metric_dict["val/avg_turns"] = np.mean(all_turn_counts)
            metric_dict["val/max_turns"] = np.max(all_turn_counts)
            metric_dict["val/min_turns"] = np.min(all_turn_counts)
            metric_dict["val/std_turns"] = np.std(all_turn_counts)

        # Print summary of additional metrics
        print("\n" + "=" * 20 + "Additional Validation Metrics" + "=" * 20)
        if all_tool_call_counts:
            print(f"Average tool calls: {np.mean(all_tool_call_counts):.2f}")
            print(f"Max tool calls: {np.max(all_tool_call_counts)}")
            print(f"Min tool calls: {np.min(all_tool_call_counts)}")
        if all_code_lines:
            print(f"Average code lines: {np.mean(all_code_lines):.2f}")
            print(f"Max code lines: {np.max(all_code_lines)}")
            print(f"Min code lines: {np.min(all_code_lines)}")
        if all_response_lengths:
            print(f"Average response length (chars): {np.mean(all_response_lengths):.2f}")
        if all_response_length_tokens:
            print(f"Average response length (tokens): {np.mean(all_response_length_tokens):.2f}")
        if all_turn_counts:
            print(f"Average turns: {np.mean(all_turn_counts):.2f}")
        print("=" * 60 + "\n")
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    attach_all_tools_failed_flags(batch)
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        attach_all_tools_failed_flags(batch, extra_rewards_info)
                        ...
                        mask_all_tools_failed_trajectories(
                            ...
                            filter_config=self.config.trainer.get(
                                "exec_error_filter", None
                            ),
                        ...
                        # begin: attach token-level causal influence after advantage
                        ce_cfg = self.config.actor_rollout_ref.actor.get("causal_entropy", {})
                        if ce_cfg.get("enable", False):
                            if ce_cfg.get("use_trajectory_selection", False):
                                metrics.update(attach_traj_explore_mask(batch, ce_cfg))
                            else:
                                metrics.update(attach_causal_influence(batch, ce_cfg))
                        # end
