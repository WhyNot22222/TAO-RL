...
class AgentHelper:
    ...
    def _update_right_side(
        ...
        right_side: Dict,
        cur_responses: torch.Tensor,
        cur_post_tool_mask: torch.Tensor,
        ...
        if next_obs_ids != None:
            ...
            # begin: concatenate post-tool mask with zero mask for observation tokens
            responses_post_tool_mask, _ = self._info_masked_concatenate_with_padding(
                right_side["responses_post_tool_mask"],
                right_side["responses_post_tool_mask"],
                cur_post_tool_mask,
                torch.zeros_like(next_obs_ids),
                pad_to_left=False,
            )
            # end
        else:
            ...
            # begin: concatenate post-tool mask without observation tokens
            responses_post_tool_mask, _ = self._info_masked_concatenate_with_padding(
                right_side["responses_post_tool_mask"],
                right_side["responses_post_tool_mask"],
                cur_post_tool_mask,
        ...
        return {
            ...
            # begin: keep post-tool mask aligned with responses
            "responses_post_tool_mask": responses_post_tool_mask[:, :max_len],
            # end
        }
    ...
    def run_llm_loop(
        ...
        original_right_side = {
            ...
            # begin: track whether response tokens are generated after tool call
            "responses_post_tool_mask": initial_input_ids[:, []],
            # end
        }
        ...
        rollings = gen_batch
        # begin: previous-step tool-call flags, used for one-step delayed post-tool mask
        prev_tool_call_flags = None
        # end
        ...
        for step in range(self.config.max_turns):
            ...
            # begin: mark tokens generated after tool feedback (one-step delay)
            cur_post_tool_mask = torch.zeros_like(responses_ids)
            if prev_tool_call_flags is not None:
                cur_post_tool_mask = torch.tensor(
                    prev_tool_call_flags,
                    dtype=responses_ids.dtype,
                    device=responses_ids.device,
                )
                cur_post_tool_mask = cur_post_tool_mask.view(-1, 1).expand_as(
                    responses_ids
                )
            # end
            ...
            # begin: current-step tool-call flags become next-step post-tool flags
            prev_tool_call_flags = code_info["use_code"]
            # end
            ...
            original_right_side = self._update_right_side(
                original_right_side, responses_ids, cur_post_tool_mask, next_obs_ids
            )
    ...
    def _compose_final_output(
        ...
        # begin: expose post-tool token mask for actor training
        final_output["post_tool_mask"] = torch.cat(
            [
